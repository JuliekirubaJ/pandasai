...
class Agent:
    ...
    def clarification_questions(self, query: str) -> List[str]:
        prompt = ClarificationQuestionPrompt(context=self.context, query=query)
        result = self.call_llm_with_prompt(prompt)
        self.logger.log(f"Clarification Questions:  {result}")

        try:
            result = result.replace("```json", "").replace("```", "").strip()  # Ensure no extra characters
            questions: List[str] = json.loads(result)
            return questions
        except json.JSONDecodeError as e:
            self.logger.log(f"Failed to decode JSON: {e}")
            return []
